...
// Tunnel represents ssh tunnel to kubernetes pod
type Tunnel struct {
	Local     int
	Remote    int
	Namespace string
...
// NewTunnel creates new tunnel to the pod
...
// Close disconnects tunnel connection
func (t *Tunnel) Close() {
	close(t.stopChan)
}
...
func (t *Tunnel) ForwardPort() error {
	...
	if err != nil {
		return fmt.Errorf("could not find available port: %s", err)
	}
	t.Local = local
